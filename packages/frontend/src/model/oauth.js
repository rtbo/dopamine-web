  ...
  async function authenticate(provider) {
    try {
      ...
      if (params.state !== state) {
        throw new Error(`OAuth ${providerConfig.name}: state doesn't match`)
      }
      ...
      try {
        ...
        if (popupPath === redirectUrlPath) {
          if (popup.location.search) {
            const params = parseQueryString(popup.location.search.substring(1))
            resolve(params)
          } else {
            reject(new Error('No query param in the OAuth redirection'))
          }
          clearInterval(poolingInterval)
          popup.close()
        }
